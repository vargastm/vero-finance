...
function ConfirmCodeContent() {
  ...
  const currencyCode = searchParams.get('currencyCode') || 'USDC'
      ...
      const response = await fetch('/api/send-withdraw-crypto-email', {
        ...
        body: JSON.stringify({
          ...
          feeAmount: searchParams.get('feeAmount')
            ? parseFloat(searchParams.get('feeAmount') || '0')
            : undefined,
          receiveAmount: searchParams.get('receiveAmount')
            ? parseFloat(searchParams.get('receiveAmount') || '0')
            : undefined,
          networkName: networkName || '',
          networkSymbol: searchParams.get('networkSymbol') || '',
          walletAddress: walletAddress || '',
          gasFee: searchParams.get('gasFee') || '',
          userEmail,
          userName: firstName,
        }),
      })
  ...
  const handleConfirmWithdraw = async () => {
    ...
    try {
      ...
      try {
        await fetch('/api/send-withdraw-crypto-confirmation', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  // Build back URL with all necessary params
  const backUrl = useMemo(() => {
    const params = new URLSearchParams()
    if (networkId) params.set('network', networkId)
    if (amount) params.set('amount', amount)
    return `/select-destination/withdraw-crypto/confirm${params.toString() ? `?${params.toString()}` : ''}`
  }, [networkId, amount])
