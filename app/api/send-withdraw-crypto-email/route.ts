...
export async function POST(request: Request) {
  try {
    ...
    // Generate confirmation code (use amount or default to 0 for crypto)
    const withdrawalAmount = amount || 0
    const cryptoCurrencyCode = currencyCode || 'USDC'
    const confirmationCode = createWithdrawCode(
      userEmail,
      withdrawalAmount,
      cryptoCurrencyCode,
    )
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [userEmail],
      subject: `Confirmation Code - Cryptocurrency Withdrawal${amount ? ` ${currencySymbol || ''}${amount} ${cryptoCurrencyCode}` : ''}`,
      html: `
        <!DOCTYPE html>
        <html>
          <head>
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e0e0e0;">
              ...
              <p>You requested a cryptocurrency withdrawal${amount ? ` of <strong>${currencySymbol || ''}${amount.toFixed(2)} ${cryptoCurrencyCode}</strong>` : ''}.</p>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                  <tr>
                    ...
                    <td style="padding: 8px 0; text-align: right; font-weight: bold; color: #333;">
                      ${currencySymbol || ''}${amount.toFixed(2)} ${cryptoCurrencyCode}
                    </td>
                  </tr>
                  ${
                    feeAmount
                  ...
                  <tr style="border-top: 2px solid #e0e0e0;">
                    ...
                    <td style="padding: 12px 0; text-align: right; font-weight: bold; font-size: 1.1em; color: #667eea;">
                      ${currencySymbol || ''}${receiveAmount.toFixed(2)} ${cryptoCurrencyCode}
                    </td>
                  </tr>
                  `
                      : ''
